...
@section('title', 'Edit Question - Doctor Portal')
@section('page-title', 'Edit Exam Question')

@section('content')
<div class="panel-card p-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
            <h2 class="h6 mb-1">{{ $question->exam->title }}</h2>
            <div class="small text-muted">{{ $question->exam->subject->code }} - {{ $question->exam->subject->name }}</div>
        </div>
        <a href="{{ route('doctor.online-exams.show', $question->exam_id) }}" class="btn btn-sm btn-outline-secondary">Back</a>
    </div>

    <form action="{{ route('doctor.questions.update', $question->id) }}" method="POST" id="questionForm" novalidate>
        @csrf
        @method('PUT')
        ...
        <div class="row g-3">
            ...
            <div class="col-md-4">
                ...
                <input type="number" name="mark" step="0.25" min="0.25" class="form-control @error('mark') is-invalid @enderror" value="{{ old('mark', $question->mark) }}" required>
                @error('mark')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>

            <div class="col-md-4">
                <label class="form-label">Order Number</label>
                <input type="number" name="order_no" min="1" class="form-control @error('order_no') is-invalid @enderror" value="{{ old('order_no', $question->order_no) }}" required>
            ...
            <div class="col-12 d-flex gap-2">
                ...
                <a href="{{ route('doctor.online-exams.show', $question->exam_id) }}" class="btn btn-outline-secondary">Cancel</a>
            </div>
        </div>
    </form>
</div>
...
(() => {
    ...
    const existingChoices = @json($question->choices->map(fn ($choice) => ['text' => $choice->choice_text, 'is_correct' => $choice->is_correct])->values());
    const oldChoices = @json(old('choices'));
    const oldCorrect = @json(old('correct_choice'));

    const dataChoices = Array.isArray(oldChoices) ? oldChoices.map((text, index) => ({ text, is_correct: Number(oldCorrect) === index })) : existingChoices;
    ...
    const addChoice = (value = '', isChecked = false) => {
        ...
        row.innerHTML = `
            ...
            <div class="col-9">
                <input type="text" class="form-control" data-choice-input value="${String(value).replace(/"/g, '&quot;')}" required>
            </div>
    ...
    if (dataChoices.length) {
        dataChoices.forEach((choice) => addChoice(choice.text ?? '', Boolean(choice.is_correct)));
    } else {
        addChoice('', true);
        addChoice('', false);
    }
